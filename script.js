...
document.addEventListener('DOMContentLoaded', function() { 
  ...
  if (phoneHeaderBtn) { 
    phoneHeaderBtn.addEventListener('click', function() { 
      const phoneNumber = '+972543951940'; 
    ...
    contactForm.addEventListener('submit', async function(e) { 
      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const zappyPayload = {
            websiteId: 'cce33087-8529-41c1-9cff-8591ff8bd700',
            name: formDataObject.name || formDataObject.fullName || formDataObject.fullname || '',
            email: formDataObject.email || '',
            subject: formDataObject.subject || 'Contact Form Submission',
            message: formDataObject.message || formDataObject.comments || formDataObject.details || '',
            phone: formDataObject.phone || formDataObject.telephone || formDataObject.mobile || null
          };
          ...
          if (response.ok) {
            ...
            console.log('⚠️ Zappy: Server responded with status', response.status);
          }
        } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "DJ Barry משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
